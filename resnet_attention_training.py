...
class ConvResBlockSE(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.se(h) # Apply Squeeze-and-Excitation
        return F.relu(x + h)

# --- ResNet with SE blocks ---
class ResNet2DWithParamsSE(nn.Module):
    """
    ResNet2D model with Squeeze-and-Excitation blocks.
    """
    def __init__(self,
                 k_max=6, nk_max=6,
                 n_params=3,
        ...
        self.blocks = nn.Sequential(
            *[ConvResBlockSE(base_ch) for _ in range(num_blocks)] # Using new SE block
        )
...
if __name__ == '__main__':
    ...
    # --- Instantiate the ResNet2DWithParamsSE model ---
    print("\n--- Initializing ResNet2DWithParamsSE model ---")
    resnet_se_model = ResNet2DWithParamsSE(
        k_max=max_k,
        nk_max=max_nk,
        n_params=3,
        base_ch=32,
    ...
    model_save_path = "best_resnet2d_se_oop.pth"
    if os.path.exists(model_save_path):
        print(f"Loading weights from {model_save_path} to continue training...")
        resnet_se_model.load_state_dict(torch.load(model_save_path, map_location=device))
    else:
    ...
    optimizer = torch.optim.AdamW(resnet_se_model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    ...
    model_name = "ResNet2D_SE_OOP_Continued"
