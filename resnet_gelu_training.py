...
# --- Modified ConvResBlock with GELU ---
class ConvResBlockGELU(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.gelu(x + h)

# --- Modified ResNet2DWithParams with GELU ---
class ResNet2DWithParamsGELU(nn.Module):
    """
    ...
    def __init__(self,
        ...
        self.blocks = nn.Sequential(
            *[ConvResBlockGELU(base_ch) for _ in range(num_blocks)] # Using new GELU block
        )
        flat_dim = base_ch * k_max * nk_max
        self.param_proj = nn.Linear(n_params, 64)
        self.param_proj_act = nn.GELU() # Changed from F.relu
...
if __name__ == '__main__':
    ...
    # --- Instantiate the ResNet2DWithParamsGELU model ---
    print("\n--- Initializing ResNet2DWithParamsGELU model ---")
    resnet_gelu_model = ResNet2DWithParamsGELU(
        k_max=max_k,
        nk_max=max_nk,
        n_params=3,
        base_ch=32,
    ...
    optimizer = torch.optim.AdamW(resnet_gelu_model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    ...
    model_name = "ResNet2D_GELU_OOP"
    trainer = ResNetTrainer(
        model=resnet_gelu_model,
