...
# --- Modified ConvResBlock with LeakyReLU ---
class ConvResBlockLeakyReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply LeakyReLU to the sum
        return self.leaky_relu(x + h)

# --- Modified ResNet2DWithParams with LeakyReLU ---
class ResNet2DWithParamsLeakyReLU(nn.Module):
    """
    ...
    def __init__(self,
        ...
        self.stem = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True) # Changed from ReLU
        )
        self.blocks = nn.Sequential(
            *[ConvResBlockLeakyReLU(base_ch) for _ in range(num_blocks)] # Using new LeakyReLU block
        )
        flat_dim = base_ch * k_max * nk_max
        self.param_proj = nn.Linear(n_params, 64)
        self.param_proj_act = nn.LeakyReLU(inplace=True) # Changed from F.relu
...
if __name__ == '__main__':
    ...
    try:
        ...
        combined_dataset = PickleFolderDataset(
            ...
            max_k=max_k,
            max_nk=max_nk,
            p_normaliser="none", # Use "none" as ResNet in notebook uses no normalization
        )
    ...
    # --- Instantiate the ResNet2DWithParamsLeakyReLU model ---
    print("\n--- Initializing ResNet2DWithParamsLeakyReLU model ---")
    resnet_leakyrelu_model = ResNet2DWithParamsLeakyReLU(
        k_max=max_k,
        nk_max=max_nk,
        n_params=3,
        base_ch=32,
    ...
    optimizer = torch.optim.AdamW(resnet_leakyrelu_model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    ...
    model_name = "ResNet2D_LeakyReLU_OOP"
    trainer = ResNetTrainer(
        model=resnet_leakyrelu_model,
