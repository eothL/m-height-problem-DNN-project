...
# --- Original ResNet Model Definitions (with ReLU) ---
...
class ResNet2DWithParams(nn.Module):
    """
    The original ResNet2D model interpreting padded P as a single-channel image.
    Modified to support Strategy 1a: Enforcing Lower Bound (h >= 1).
    """
    def __init__(self,
                 k_max=6, nk_max=6,
                 n_params=3,
    ...
    def forward(self, P, params):
        ...
        z = self.head(x) # Get raw output from the head
...
class ResNetTrainer:
    def __init__(self, model, train_loader, val_loader, criterion, optimizer,
        ...
        self.is_violation_loss = isinstance(criterion, ViolationInformedLossAccelerated)
...
if __name__ == '__main__':
    ...
    # --- Best Hyperparameters from Optuna for SE-ResNet (as a starting point) ---
    BEST_PARAMS = {
    ...
    final_model = ResNet2DWithParams(
        ...
        nk_max=max_nk,
        n_params=3,
        base_ch=BEST_PARAMS["base_ch"], # Using best params from SE-ResNet
        num_blocks=BEST_PARAMS["num_blocks"], # Using best params from SE-ResNet
    ...
    model_name = "ReLU_ResNet_LowerBound_ViolationLoss"
