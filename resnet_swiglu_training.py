...
# --- Training Setup for ResNet2DWithParamsSwiGLU ---
if __name__ == '__main__':
    ...
    resnet_swiglu_model = ResNet2DWithParamsSwiGLU(
        ...
        nk_max=max_nk,
        n_params=3,
        base_ch=32, # Using base_ch from original ResNet2DWithParams
        num_blocks=3 # Using num_blocks from original ResNet2DWithParams
    ).to(device)

    # --- Loss & Optimizer ---
    criterion = LogMSELoss()
    optimizer = torch.optim.AdamW(resnet_swiglu_model.parameters(), lr=3e-4, weight_decay=1e-4) # Example values

    # --- Scheduler ---
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5, factor=0.2, verbose=True)

    # --- Run Training ---
    model_name = "ResNet2D_SwiGLU"
    ...
    # --- Record & Plot Results ---
    results = {} # Assuming 'results' dict is used for plotting
